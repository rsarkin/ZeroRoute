...
import { History, TrendingDown, Plus } from 'lucide-react';
import { EmissionCategory } from '../../types';

export default function Dashboard() {
  const router = useRouter();
  const { familyProfile, members, emissionLogs, addLog, badges } = useFamily();
  ...
  const handleQuickLogLastTrip = () => {
    ...
    const userLogs = emissionLogs.filter(
      (l) => l.memberId === selectedMemberId && l.category === 'transport'
    );
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between border border-emerald-900/50 bg-forest-green rounded-xl p-6 shadow-xs text-white">
        <div>
          ...
          <p className="text-sm text-emerald-200/80 mt-1">
            Track and nurture your family forest. Currently simulating as{' '}
            <span className="font-semibold text-[#ebdcb9]">{user?.displayName || 'Guest'}</span>.
          </p>
        </div>
        {actualReductionPercent > 0 && (
          <div className="mt-4 md:mt-0 flex items-center gap-3 bg-emerald-950/50 text-emerald-200 border border-emerald-800/60 px-4 py-3 rounded-lg">
            ...
            <div>
              ...
              <p className="text-sm font-semibold text-[#ebdcb9]">
                {actualReductionPercent.toFixed(1)}% below baseline
              </p>
            </div>
          </div>
        )}
          ...
          <div className="space-y-3">
            <div className="flex justify-between items-center">
              ...
              <span className="text-xs text-slate-500 font-medium">
                Trees Planted: {1 + badges.length}
              </span>
            </div>
            <ForestSvg reductionPercent={actualReductionPercent} badges={badges} />
          </div>
          ...
          <div className="notion-card space-y-4">
            ...
            <div>
              <label
                ...
                className="block text-xs font-semibold text-slate mb-1"
              >
                Logging For
              </label>
              <select
            ...
            {/* Date selector */}
            <div>
              <label htmlFor="log-date" className="block text-xs font-semibold text-slate mb-1">
                Date
              </label>
              <input
                id="log-date"
            ...
            {activeTab === 'transport' && (
              <form onSubmit={handleTransportSubmit} className="space-y-4 pt-2">
                ...
                <div className="flex gap-2">
                  <button type="submit" className="notion-btn-primary flex-1 text-sm font-semibold">
                    Log Trip
                  </button>
                  <button
                    type="button"
            ...
            {activeTab === 'energy' && (
              <form onSubmit={handleEnergySubmit} className="space-y-4 pt-2">
                ...
                <button type="submit" className="notion-btn-primary w-full text-sm font-semibold">
                  Log Energy Use
                </button>
              </form>
            )}
