...
import { Zap, ShieldAlert, ArrowDown, ArrowUp } from 'lucide-react';
...
export const DashboardStats: React.FC = () => {
  ...
  const strokeDashoffset =
    circumference - (Math.min(budgetSpentPercent, 100) / 100) * circumference;
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-1 gap-4">
        ...
        <div className="notion-card flex flex-col justify-between">
          <div>
            <h3 className="text-xs font-semibold text-steel uppercase tracking-wider">
          ...
          <div className="mt-4 text-xs text-slate">
            {co2Saved > 0 ? (
              ...
              <span className="text-slate-500">Baseline: {baselineCo2} kg</span>
            )}
          </div>
        </div>
      ...
      {co2Saved > 0 && (
        <div className="p-4 bg-card-tint-mint border border-brand-green/30 rounded-lg">
          ...
          <p className="text-sm text-slate-800 font-medium leading-relaxed">
            By saving <span className="font-bold text-brand-green">{co2Saved} kg of CO₂</span> this
            week, your family has:
          </p>
          <ul className="mt-2 space-y-1 text-xs text-slate-700 list-disc list-inside">
            <li>
              Saved equivalent of driving{' '}
              <span className="font-semibold">{equivalents.carKms} km</span> in a petrol SUV.
            </li>
            <li>
              Avoided emissions comparable to charging{' '}
              <span className="font-semibold">{equivalents.phoneCharges.toLocaleString()}</span>{' '}
              smartphones.
            </li>
            <li>
              Equal to <span className="font-semibold">{equivalents.treeWeeks} weeks</span> of
              Carbon absorption by a mature tree.
            </li>
          </ul>
        </div>
      )}
